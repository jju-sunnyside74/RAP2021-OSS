...
def learning_decay(in_data, fs, tgt_rt=None, use_rt60=False):
	...
	# # Calculation Normalized Decay Curve & Estimation Time
	# decaycurve = numpy.float32(room.decayCurve(data, estimate_time, fs))
	...
	# # Calculation Normalized Decay Curve & Estimation Time
	# decaycurve_temp = numpy.float32(room.decayCurve(data, estimate_time_temp, fs))
	# estimate_time, impulse_t30nonLin, s_0dB_temp, s_10dB_temp, s_20dB_temp, s_30dB_temp = room.T30_learning(decaycurve_temp, fs)

	# decaycurve = numpy.float32(room.decayCurve(data, estimate_time[0][0]*2, fs))
