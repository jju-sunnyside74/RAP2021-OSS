...
def calc_filt_impulse_learning(draw_plot, in_data, fs, fc, filt_type='butt', order_tab=2, RT60=False, fname = "Please set file name"):
    ...
    # Plot Filtered Impulse Data
    if draw_plot:
        dbg.dPlotAudio('impluse', fs, data_filtered, fname + ' filtered ' + str(fc) + 'Hz', str_ch_name, "Time(sec)", "Amplitude")
    ...
    # Plot DecayCurve
    if draw_plot:
        dbg.dPlotAudio('decay', fs, decaycurve, fname + ' decay curve ' + str(fc) + 'Hz', str_ch_name, "Time(sec)", "Amplitude")
    ...
    #print("...impulse_t30nonLin(calc_filt_impulse_learning) ", impulse_t30nonLin)
